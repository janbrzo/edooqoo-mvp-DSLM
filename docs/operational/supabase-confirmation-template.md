...
**Personalization**: `{{ .Data.first_name }}` reads `raw_user_meta_data.first_name` set during `signUp({ options: { data: { first_name } } })` in `src/pages/Signup.tsx`. If absent, falls back to "Confirm your email".

**Why no `hello@edooqoo.com` in footer**: mailbox does not exist; Supabase auth emails reply-to defaults to `noreply@mail.app.supabase.io`. Showing the address misleads users.
