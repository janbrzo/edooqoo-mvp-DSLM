...
export const GoalsView: React.FC<GoalsViewProps> = ({
  ...
  // v6.9.39 P5 — suggested goals auto-generated by process-welcome-test.
  ...
  const suggestedGoals = goals.filter(
    (g: any) => g.source === 'welcome_test_auto' && !g.accepted_at && isActive(g),
  );
  ...
  const acceptAllSuggested = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .in('id', suggestedGoals.map((g: any) => g.id));
  ...
  const dismissAllSuggested = async () => {
    ...
    try {
      await Promise.all(suggestedGoals.map((g: any) => deleteGoal(g.id)));
