...
export const GoalsView: React.FC<GoalsViewProps> = ({
  ...
  // v6.9.47 P5 — suggested goals auto-generated by process-welcome-test.
  // We treat any active goal with source='welcome_test_auto' and no
  // accepted_at as a suggestion awaiting teacher decision.
  // Optimistic state: locally-accepted/dismissed ids hide the suggestion
  // immediately, before the realtime refetch lands.
  const [optimisticAccepted, setOptimisticAccepted] = React.useState<Set<string>>(new Set());
  const [optimisticDismissed, setOptimisticDismissed] = React.useState<Set<string>>(new Set());
  const suggestedGoals = goals.filter(
    (g: any) => g.source === 'welcome_test_auto'
      && !g.accepted_at
      && !optimisticAccepted.has(g.id)
      && !optimisticDismissed.has(g.id)
      && isActive(g),
  );
  ...
  const acceptAllSuggested = async () => {
    ...
    const ids = suggestedGoals.map((g: any) => g.id);
    ...
    try {
      ...
      // useStudentProgress re-fetches on its realtime channel; force a soft
      // refresh fallback in case the subscription is delayed.
      window.dispatchEvent(new CustomEvent('student-progress:refresh'));
    } catch (err) {
      console.error('acceptAllSuggested failed', err);
  ...
  const dismissAllSuggested = async () => {
    ...
    try {
      ...
      console.error('dismissAllSuggested failed', err);
