...
export const SpotlightOverlay: React.FC = () => {
  ...
  }, [location.pathname, searchParams.get('focus')]);
  ...
  const dim = 'fixed bg-black/60 pointer-events-auto transition-opacity duration-200';

  return createPortal(
    <div className="fixed inset-0 z-[100]" aria-live="polite">
      {/* top */}
      <div className={dim} style={{ top: 0, left: 0, width: vw, height: rect.top }} onClick={clear} />
      {/* bottom */}
      <div className={dim} style={{ top: rect.top + rect.height, left: 0, width: vw, height: Math.max(0, vh - (rect.top + rect.height)) }} onClick={clear} />
      {/* left */}
      <div className={dim} style={{ top: rect.top, left: 0, width: rect.left, height: rect.height }} onClick={clear} />
      {/* right */}
      <div className={dim} style={{ top: rect.top, left: rect.left + rect.width, width: Math.max(0, vw - (rect.left + rect.width)), height: rect.height }} onClick={clear} />
      ...
      {/* Hint tooltip */}
      <div
        className="fixed px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-xs font-medium shadow-lg pointer-events-none"
        style={{
          top: Math.min(vh - 36, rect.top + rect.height + 8),
          left: Math.min(vw - 180, Math.max(8, rect.left)),
        }}
      >
        👉 Click here to continue · ESC to dismiss
      </div>
    </div>,
    document.body,
  );
