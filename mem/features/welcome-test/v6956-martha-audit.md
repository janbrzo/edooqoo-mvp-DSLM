...
- **Wave 3 (done — partial)**: IDK signal aggregation added in `process-welcome-test/index.ts` — counts `__IDK__` answers (total + skill-only) and derives `self_awareness_score = idkCountSkill / (skillIDK + skillWrong) * 100`. New columns `idk_count_total`, `idk_count_skill`, `self_awareness_score` written on every upsert. AI summary prompt now receives an explicit IDK signal line and an Integrity signal line built from the new `__integrity__` snapshot. Universal IDK button: visibility condition relaxed from `isSkillQuestion` to "any unanswered question" with context-aware tooltip. `useWelcomeTestIntegrity` hook ships, but persists tab-blur events to `localStorage` keyed by `wt_integrity_<testId>` (anon RLS blocks direct `student_events` inserts) — `consumeWelcomeTestIntegrity()` drains the buffer in `useWelcomeTest.completeTest` and forwards it as `integrity` in the `process-welcome-test` payload, which stores it under `raw_answers.__integrity__` via service role. Paste blocker active on textarea / text input elements while open-ended question is shown.
- **Wave 3 (done — deferred items resolved)**: TTS regeneration for `wt_q18l` completed via `generate-welcome-test-audio` — new asset `welcome-test-listening-1781265449193.mp3` now referenced from `welcomeTestQuestions.ts`; legacy 1771235244954 mp3 retained for historical answer snapshots. Retranslation of the 4 non-skill changed questions (q1/q3/q6/q7) across 9 locales (ru/uk/es/fr/de/pt/it/zh/ja) executed via Lovable AI Gateway (`google/gemini-3-flash-preview`) and patched into `welcomeTestTranslations.ts`. q16s/q18/q18l/q21/q22/q35 remain English-only per the existing skill-question rule. The remaining 15 language sets (tr/cs/nl/ko/ar/hu/ro/el/hr/sv/hi/vi/th/no/da) keep their prior translations which still convey the same intent — re-translation only required if a future audit flags a specific drift.
