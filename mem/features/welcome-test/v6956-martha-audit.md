...
- **Wave 3 (deferred — explicit)**: TTS regeneration for the new `wt_q18l` (restaurant booking) transcript via `generate-welcome-test-audio` — `audio_url` still points to the legacy 1771235244954 mp3; ListeningPlayer falls back to transcript. Retranslation of 8 changed questions across the remaining 9 locales (ru/uk/es/fr/de/pt/it/zh/ja) via Lovable AI Gateway batch. Both items tracked separately as future ad-hoc runs — they are not blocked by anything in code; only the assets/strings need refresh.
